...
class MessagingTest : public CppUnit::TestFixture {
        ...
        void test_messaging()
	{
	    ...
	    //connect the two
	    Manifold :: ConnectHalf(c0, MyObj1<T>::C0_OUT, c1, MyObj1<T>::C1_IN, &MyObj1<T>::process_incoming, MyObj1<T>::C0_OUT_LATENCY);
		...
		Manifold :: ScheduleHalf(when[i], &MyObj1<T>::handler, comp, send_data[i]);
	    }
