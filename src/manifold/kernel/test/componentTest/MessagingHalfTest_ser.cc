...
//!
//! In this program, the components are clock based. The clock used is the master clock.
...
//! In this program data is passed by value.
//!
...
class MyObj1 : public Component {
	...
	{
	    #ifdef DBG
	    Ticks_t now = Manifold :: NowHalfTicks();

	    m_dbg_log << "@@@ " << Manifold :: NowTicks() << " (half tick " << now << ")" << endl;
	    #endif
	...
	{
	    #ifdef DBG
	    m_dbg_log << "@@@ " << Manifold :: NowTicks() << " (half tick " << Manifold::NowHalfTicks() << ")" << endl;
	    ...
	    m_recvHalfTick.push_back(Manifold :: NowHalfTicks());
	}
};
...
class MessagingTest : public CppUnit::TestFixture {
        ...
        void test_messaging()
	{
	    ...
	    #ifdef DBG
	    CompId_t c0 = Component :: Create<MyObj1>(0, DBG_LOG); //c0 created for LP 0
	    CompId_t c1 = Component :: Create<MyObj1>(1, DBG_LOG); //c1 created for LP 1
	    #else
	    ...
	    //connect the two
	    Manifold :: ConnectHalf(c0, MyObj1::C0_OUT, c1, MyObj1::C1_IN, &MyObj1::process_incoming, MyObj1::C0_OUT_LATENCY);
		...
		Manifold :: ScheduleHalf(when[i], &MyObj1::handler, comp, send_data[i]);
	    }
	    const unsigned int STOP = 1000;
	    CPPUNIT_ASSERT(when[SIZE-1] < STOP);

	    Ticks_t ScheduleAt = Manifold :: NowHalfTicks(); // Tick time when we schedule the events.
