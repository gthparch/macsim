...
class MessagingTest : public CppUnit::TestFixture {
        ...
        void test_messaging()
	{
	    ...
	    //connect the two
	    Manifold :: Connect(c0, MyObj1<T>::C0_OUT, c1, MyObj1<T>::C1_IN, &MyObj1<T>::process_incoming, MyObj1<T>::C0_OUT_LATENCY);
		...
		Manifold :: Schedule(when[i], &MyObj1<T>::handler, comp, send_data[i]);
	    }
	    const unsigned int STOP = 1000;
	    CPPUNIT_ASSERT(when[SIZE-1] < STOP);

	    Ticks_t ScheduleAt = Manifold :: NowTicks(); // Tick time when we schedule the events.
	...
	static CppUnit::Test* suite()
	{
	    ...
	    mySuite->addTest(new CppUnit::TestCaller<MessagingTest>("test_messaging_uint32", &MessagingTest::test_messaging<uint32_t>));
	    mySuite->addTest(new CppUnit::TestCaller<MessagingTest>("test_messaging_uint64", &MessagingTest::test_messaging<uint64_t>));
