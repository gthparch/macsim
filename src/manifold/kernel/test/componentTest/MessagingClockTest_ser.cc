...
class MyObj1 : public Component {
	...
	{
	    #ifdef DBG
	    Ticks_t now = Manifold :: NowTicks(m_clock);
	    m_dbg_log << "@@@ " << now << endl;
	    #endif
	...
	{
	    #ifdef DBG
	    m_dbg_log << "@@@ " << Manifold :: NowTicks(m_clock) << endl;
	    ...
	    m_recvTick.push_back(Manifold :: NowTicks(m_clock));
	}
};
...
class MessagingTest : public CppUnit::TestFixture {
        ...
        void test_messaging()
	{
	    ...
	    //connect the two
	    Manifold :: ConnectClock(c0, MyObj1::C0_OUT, c1, MyObj1::C1_IN, Clock1, &MyObj1::process_incoming, MyObj1::C0_OUT_LATENCY);
		...
		Manifold :: ScheduleClock(when[i], Clock1, &MyObj1::handler, comp, send_data[i]);
	    }
	    const unsigned int STOP = 1000;
	    CPPUNIT_ASSERT(when[SIZE-1] < STOP);

	    Ticks_t ScheduleAt = Manifold :: NowTicks(Clock1); // Tick time when we schedule the events.
		...
		//unsigned char data[SIZE][sizeof(MyPkt)];
