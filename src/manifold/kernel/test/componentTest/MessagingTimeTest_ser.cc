...
class MessagingTest : public CppUnit::TestFixture {
        ...
        void test_messaging()
	{
	    ...
	    //connect the two
	    Manifold :: ConnectTime(c0, MyObj1::C0_OUT, c1, MyObj1::C1_IN, &MyObj1::process_incoming, MyObj1::C0_OUT_LATENCY);
		...
		Manifold :: ScheduleTime(when[i], &MyObj1::handler, comp, send_data[i]);
	    }
		...
		for(int i=0; i<SIZE; i++) {
		    ...
		    CPPUNIT_ASSERT_DOUBLES_EQUAL(ScheduleAt + when[i] + MyObj1::C0_OUT_LATENCY, times[i], DOUBLE_COMP_DELTA);
		}
	    }
	    else {
