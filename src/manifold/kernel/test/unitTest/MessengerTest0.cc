//!
//! @brief This program tests Messenger :: send_uint32_msg(), send_uint64_msg(),
//! and send_serial_msg().
//!
...
class MessengerTest : public CppUnit::TestFixture {
    ...
    public:
        //==========================================================================
        //==========================================================================
        //! Test send_uint32_msg with sendTime/recvTime
	//! We send a few messages from one task to another and echo them back.
        void test_send_uint32_msg_1()
	{
	    int Mytid; //task id
		...
		uint32_t data[SIZE];
		    ...
		    data[i] = (int)(random() & 0xffffffff);

		    //send a message
		    TheMessenger.send_uint32_msg(1, cidx[i], iidx[i], when[i][0], when[i][1], data[i]);
			...
			if(received != 0) {
			    CPPUNIT_ASSERT(msg.type == Message_s :: M_UINT32);
			    ...
			    CPPUNIT_ASSERT(msg.uint32_data == data[i]);
			}
		    }while(received == 0);
		}
            }
		...
		while(num < SIZE) {
			...
			if(received != 0) {
			    ...
			    //send it back
			    TheMessenger.send_uint32_msg(0, msg.compIndex, msg.inputIndex, msg.sendTime, msg.recvTime, msg.uint32_data);
			}
		    }while(received==0);
		    num++;
		}
        ...
        //==========================================================================
        //==========================================================================
        //! Test send_uint32_msg with sendTick/recvTick
	//! We send a few messages from one task to another and echo them back.
        void test_send_uint32_msg_2()
	{
	    int Mytid; //task id
	    MPI_Comm_rank(MPI_COMM_WORLD, &Mytid);
		...
		while(num < SIZE) {
			...
			if(received != 0) {
			    ...
			    //send it back
			    TheMessenger.send_uint32_msg(0, msg.compIndex, msg.inputIndex, msg.sendTick, msg.recvTick, msg.uint32_data);
			}
		    }while(received == 0);
		    num++;
		}
        ...
        //==========================================================================
        //==========================================================================
        //! Test send_uint64_msg with sendTime/recvTime
	//! We send a few messages from one task to another and echo them back.
        void test_send_uint64_msg_1()
	{
	    int Mytid; //task id
	    MPI_Comm_rank(MPI_COMM_WORLD, &Mytid);
		...
		uint64_t data[SIZE];
		    ...
		    //send a message
		    TheMessenger.send_uint64_msg(1, cidx[i], iidx[i], when[i][0], when[i][1], data[i]);
			...
			Message_s& msg = TheMessenger.irecv_message(&received);
			if(received != 0) {
			    CPPUNIT_ASSERT(msg.type == Message_s :: M_UINT64);
			    CPPUNIT_ASSERT_EQUAL(msg.compIndex, cidx[i]);
			    CPPUNIT_ASSERT_EQUAL(msg.inputIndex, iidx[i]);
			    CPPUNIT_ASSERT_EQUAL(msg.isTick, 0);
			    CPPUNIT_ASSERT_DOUBLES_EQUAL(msg.sendTime, when[i][0], DOUBLE_COMP_DELTA);
			    CPPUNIT_ASSERT_DOUBLES_EQUAL(msg.recvTime, when[i][1], DOUBLE_COMP_DELTA);
			    CPPUNIT_ASSERT(msg.uint64_data == data[i]);
			}
		    }while(received == 0);
		}
            }
		...
		while(num < SIZE) {
			...
			if(received != 0) {
			    ...
			    //send it back
			    TheMessenger.send_uint64_msg(0, msg.compIndex, msg.inputIndex, msg.sendTime, msg.recvTime, msg.uint64_data);
			}
		    }while(received==0);
		    num++;
		}
        ...
        //==========================================================================
        //==========================================================================
        //! Test send_uint64_msg with sendTick/recvTick
	//! We send a few messages from one task to another and echo them back.
        void test_send_uint64_msg_2()
	{
	    int Mytid; //task id
	    MPI_Comm_rank(MPI_COMM_WORLD, &Mytid);
		...
		while(num < SIZE) {
			...
			if(received != 0) {
			    ...
			    //send it back
			    TheMessenger.send_uint64_msg(0, msg.compIndex, msg.inputIndex, msg.sendTick, msg.recvTick, msg.uint64_data);
			}
		    }while(received == 0);
		    num++;
		}
	...
	static CppUnit::Test* suite()
	{
	    ...
	    mySuite->addTest(new CppUnit::TestCaller<MessengerTest>("test_send_uint32_msg_1", &MessengerTest::test_send_uint32_msg_1));
	    mySuite->addTest(new CppUnit::TestCaller<MessengerTest>("test_send_uint32_msg_2", &MessengerTest::test_send_uint32_msg_2));
	    mySuite->addTest(new CppUnit::TestCaller<MessengerTest>("test_send_uint64_msg_1", &MessengerTest::test_send_uint64_msg_1));
	    mySuite->addTest(new CppUnit::TestCaller<MessengerTest>("test_send_uint64_msg_2", &MessengerTest::test_send_uint64_msg_2));
