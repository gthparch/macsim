...
void dram_controller_c::run_a_cycle()
{
  ...
  // Jaekyu's bandwidth study
  if (m_simBase->m_simulation_cycle % 100000 == 0) {
    acc_bandwidth += total_dram_bandwidth;

    total_dram_bandwidth = 0; 

    int delta = 0;
    if (m_prev_band > 0) {
      delta = m_band - m_prev_band;
      delta *= 100;
      delta /= m_prev_band;
    }

    if (m_simBase->m_simulation_cycle > 100000 && 0) {
      if (delta <= *m_simBase->m_knobs->KNOB_BDP_THRESHOLD && delta >= -*m_simBase->m_knobs->KNOB_BDP_THRESHOLD) {
        // does not reach the stable state yet
        if (m_dram_state < 5) {
          ++m_dram_state;
          m_avg_band += m_band;
          ...
          // stable state
          if (m_dram_state == 5) {
            if (bandwidth_cp_first) {
              special_adjust = *m_simBase->m_knobs->KNOB_BDP_DEGREE;
              bandwidth_cp_first = false;
              latency_dir = true; // true: decrease, false: increase
              m_prev_stable_band = m_avg_band / 5;
            }
            else {
              int band_delta = -100;

              if (m_prev_stable_band != 0) {
                static_cast<int>(m_avg_band/5) - m_prev_stable_band;
                band_delta *= 100;
                band_delta /= m_prev_stable_band;
              }
#if 0
              report("curr: " << static_cast<int>(m_avg_band/5) << " latency_delta: " << band_delta << " prev_stable: " << m_prev_stable_band << " stable_counter: " << m_stable_counter);
#endif


              bool change = true;
              // latency increase (bad)
              if (band_delta >= *m_simBase->m_knobs->KNOB_BDP_THRESHOLD) {
                m_stable_counter = 0;
              ...
              // latency decrease (good)
              else if (band_delta <= -*m_simBase->m_knobs->KNOB_BDP_THRESHOLD) {
                m_stable_counter = 0;
                if (!latency_dir) {
                  special_adjust -= *m_simBase->m_knobs->KNOB_BDP_DEGREE;
                }
                ...
                // in stable state for a while, so need action
                if (m_stable_counter >= 3) {
                  m_stable_counter = 0;
                  if (latency_dir) {
                    special_adjust += *m_simBase->m_knobs->KNOB_BDP_DEGREE;
                  }
...
#if 0
    report("band: " << m_band << " prev: " << m_prev_band << " delta: " << delta << " state: " << m_dram_state << " dir: " << latency_dir << " val: " << special_adjust << " last_stable: " << m_prev_stable_band);
    temp_out << "band: " << m_band << " prev: " << m_prev_band << " delta: " << delta << " state: " << m_dram_state << " dir: " << latency_dir << " val: " << special_adjust << " last_stable: " << m_prev_stable_band << "\n";
#endif
    m_prev_band = m_band;
    m_band = 0;
  }
