...
void hmc_function_c::hmc_info_read(string file_name_base, map<uint64_t, hmc_inst_s> &m_hmc_info,
...
void hmc_function_c::hmc_fence_info_read(string file_name_base, set<uint64_t> &m_hmc_fence_info) {
  ifstream hmc_info_file;
  string hmc_info = file_name_base + ".HMCinfo";
  hmc_info_file.open(hmc_info.c_str());
...
void hmc_function_c::lock_info_read(string file_name_base, map<uint64_t, hmc_inst_s> &m_lock_info) {
...
HMC_Type hmc_function_c::generate_hmc_inst(const hmc_inst_s &inst_info, uint64_t hmc_vaddr,
...
bool hmc_function_c::get_uops_from_traces_with_hmc_inst(void *ptr, int core_id, uop_c *uop, int sim_thread_id) {
  ...
  if (thread_trace_info->m_bom) {
    ...
    if (*KNOB(KNOB_COUNT_HMC_REMOVED_IN_MAX_INSTS))
      inst_extra =
        (m_simBase->m_ProcessorStats->core(core_id))[HMC_REMOVE_INST_COUNT - PER_CORE_STATS_ENUM_FIRST].getCount();

    if ((core->m_inst_fetched[sim_thread_id] + inst_extra) < *KNOB(KNOB_MAX_INSTS)) {
      if (!thread_trace_info->has_cached_inst) {
        // read next instruction
        read_success =
          ((cpu_decoder_c *)ptr)->read_trace(core_id, thread_trace_info->m_next_trace_info, sim_thread_id, &inst_read);
        ...
        uint64_t inst_addr =
          (static_cast<trace_info_cpu_s *>(thread_trace_info->m_next_trace_info))->m_instruction_addr;
        if (hmc_info.find(inst_addr) != hmc_info.end() && (!core->get_trace_info(sim_thread_id)->m_trace_ended)) {
          ...
          if (!core->get_trace_info(sim_thread_id)->m_trace_ended) {
            ...
            DEBUG_CORE(core_id,
                       "[HMC] core_id:%d cycle_count:%lld new trace info is created pc:%lx va:%lx"
                       "return_type:%d thread_trace_info->m_next_hmc_type:%d \n",
                       core_id, core->get_cycle_count(), tmp_trace_info->m_instruction_addr,
                       tmp_trace_info->m_ld_vaddr1, ret, thread_trace_info->m_next_hmc_type);

            DEBUG_CORE(core_id, "core_id:%d thread_id:%d hmc_inst:%d m_next_hmc_type:%d\n", core_id, sim_thread_id, ret,
                       thread_trace_info->m_next_hmc_type);
        ...
        memcpy(thread_trace_info->m_next_trace_info, &(thread_trace_info->cached_inst), sizeof(trace_info_cpu_s));
        inst_read = true;
        read_success = true;
    ...
    DEBUG_CORE(core_id, "trace_read core_id:%d thread_id:%d pc:0x%llx opcode:%d inst_count:%llu va:0x%lx\n", core_id,
               ...
               (Counter)(thread_trace_info->m_temp_inst_count), (trace_info.m_ld_vaddr1));
    ...
    info = ((cpu_decoder_c *)ptr)
             ->convert_pinuop_to_t_uop(&trace_info, thread_trace_info->m_trace_uop_array, core_id, sim_thread_id);
  ...
  if (uop->m_hmc_inst != HMC_NONE) {
    DEBUG_CORE(core_id, "core_id:%d thread_id:%d inst_num:%lld uop_num:%lld pc:%llx va:%llx\n", core_id, sim_thread_id,
               uop->m_inst_num, uop->m_uop_num, trace_uop->m_addr, trace_uop->m_va);
  ...
  if (uop->m_num_srcs < MAX_SRCS) {
    for (int index = 0; index < uop->m_num_srcs; ++index) {
      ...
      DEBUG_CORE(
        uop->m_core_id,
        "thread_id:%d uop_num:%llu unique_num:%lld src_info[%d]:%u last_dest_reg:%d dest_id[%d]:%u m_num_dests:%d\n",
        uop->m_thread_id, uop->m_uop_num, uop->m_unique_num, index, uop->m_src_info[index],
        thread_trace_info->m_last_dest_reg, index, uop->m_dest_info[index], uop->m_num_dests);
    }
  } else {
  ...
  for (int index = 0; index < uop->m_num_dests; ++index) {
    ...
    DEBUG_CORE(uop->m_core_id, "unique_num:%lld dest_info[%d]:%u last_dest_reg:%u\n", uop->m_unique_num, index,
               uop->m_dest_info[index], thread_trace_info->m_last_dest_reg);
  ...
  if (uop->m_hmc_inst > HMC_NONE && uop->m_hmc_inst <= HMC_CAS_less_16B) {
    DEBUG_CORE(core_id, "-------core_id:%d thread_id:%d inst_num:%lld uop_num:%lld pc:%llx va:%llx\n", core_id,
               sim_thread_id, uop->m_inst_num, uop->m_uop_num, trace_uop->m_addr, trace_uop->m_va);
    // add-dep
    uop->m_num_dests = 1;
  ...
  DEBUG_CORE(
    uop->m_core_id,
    "new uop: uop_num:%lld inst_num:%lld thread_id:%d unique_num:%lld src[0]:%d dst[0]:%d hmc_inst:%d m_num_dests:%d\n",
    uop->m_uop_num, uop->m_inst_num, uop->m_thread_id, uop->m_unique_num, uop->m_src_info[0], uop->m_dest_info[0],
    uop->m_hmc_inst, uop->m_num_dests);

  return read_success;
}
// lightweight hmc transaction support (or we call it complex hmc inst)
bool hmc_function_c::get_uops_from_traces_with_hmc_trans(void *ptr, int core_id, uop_c *uop, int sim_thread_id) {
  macsim_c *m_simBase = ((cpu_decoder_c *)ptr)->m_simBase;
  bool m_enable_physical_mapping = ((cpu_decoder_c *)ptr)->m_enable_physical_mapping;
  PageMapper *m_page_mapper = ((cpu_decoder_c *)ptr)->m_page_mapper;
  ...
  if (thread_trace_info->m_bom) {
    ...
    if (core->m_inst_fetched[sim_thread_id] < *KNOB(KNOB_MAX_INSTS)) {
      ...
      if (*KNOB(KNOB_ENABLE_LOCK_SKIP)) {
        map<uint64_t, hmc_inst_s> &lock_info = thread_trace_info->m_process->m_lock_info;
        uint64_t inst_addr =
          (static_cast<trace_info_cpu_s *>(thread_trace_info->m_next_trace_info))->m_instruction_addr;
        if (lock_info.find(inst_addr) != lock_info.end() && (!core->get_trace_info(sim_thread_id)->m_trace_ended)) {
          uint64_t ret_pc = lock_info[inst_addr].ret_pc;
          while (true) {
            read_success = ((cpu_decoder_c *)ptr)
                             ->read_trace(core_id, thread_trace_info->m_next_trace_info, sim_thread_id, &inst_read);
      ...
      if (thread_trace_info->m_inside_hmc_func == false) {
        if (hmc_info.find(inst_addr) != hmc_info.end()) {
          ...
          if (!core->get_trace_info(sim_thread_id)->m_trace_ended) {
            ...
            thread_trace_info->m_next_hmc_trans_id = m_simBase->m_hmc_trans_id_gen;
        ...
        if (inst_addr == thread_trace_info->m_next_hmc_func_ret) {
          ...
          while (next_trace->m_num_ld == 0 && next_trace->m_has_st == false) {
            ...
            next_trace = static_cast<trace_info_cpu_s *>(thread_trace_info->m_next_trace_info);
            if (core->get_trace_info(sim_thread_id)->m_trace_ended ||
                inst_addr == thread_trace_info->m_next_hmc_func_ret || next_trace->m_cf_type == CF_RET) {
              thread_trace_info->m_inside_hmc_func = false;
              thread_trace_info->m_next_hmc_type = HMC_NONE;
              break;
            }
    ...
    if (cur_trace_hmc_type != HMC_NONE && thread_trace_info->m_next_hmc_type == HMC_NONE)
      cur_trace_hmc_type = HMC_TRANS_END;

    // Copy next instruction to current instruction field
    memcpy(thread_trace_info->m_prev_trace_info, thread_trace_info->m_next_trace_info, sizeof(trace_info_cpu_s));
    thread_trace_info->m_prev_hmc_type = thread_trace_info->m_next_hmc_type;
    thread_trace_info->m_prev_hmc_trans_id = thread_trace_info->m_next_hmc_trans_id;
  ...
  if (uop->m_hmc_inst != HMC_NONE) {
    DEBUG_HMC_CORE(core_id, "[HMC] m_hmc_inst:%u id:%u mem_type:%u\n", (unsigned)uop->m_hmc_inst,
                   (unsigned)uop->m_hmc_trans_id, (unsigned)uop->m_mem_type);
  ...
  DEBUG_CORE(uop->m_core_id,
             "uop_num:%llu num_srcs:%d  trace_uop->num_src_regs:%d  num_dsts:%d num_seing_uop:%d "
             "pc:0x%llx dir:%d \n",
