...
\begin{itemize}
  ...
  traffic, but this is currently not modeled \todo{do you mean not used?}
...
\begin{itemize}
  ...
  \item Upper-level cache to input queue : forwarding of upper-level cache miss \todo{even writebacks from upper level cache can come here?}
...
with \todo{\SIM pre-defining only the links between the different levels}.
...
\SIM , the L2 cache can be disabled if needed by some configurations.
  ...
  \item All the caches \todo{what about L1?} and memory controllers are
  ...
  the static \todo{bank?} partition. In other words, address regions are
...
\ignore{
  ...
  tiles must be the same in this configuration. \todo{how is the direct link
    configured?}
...
constraints and bandwidth specifications when scheduling requests.\todo{include
  ...
  or more channels. \todo{incomplete - Each bank has its own request buffer and
...
\begin{itemize}
  ...
  \item \todo{The channel scheduler picks a request from command-ready banks
