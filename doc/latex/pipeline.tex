...
The depth of the front-end stage is set using one of the {\textit
  fetch\_latency} knobs, there is one knob for each core type (small, medium
      and large).\ignore{ and it basically sets the length of {\textit q\_fe }}
  \TODO {Different fetch policies can be implemented using the factory
    mechanism explained in Section~\ref{}. } The default fetch policy is
    round-robin and the policy to be used is specified using the
    \textit{fetch\_policy} knob. \TODO{there is one common knob for all core
      types, same for bp\_\_dir\_mech!} Different branch predictors are
