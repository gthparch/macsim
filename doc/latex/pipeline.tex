...
\TODO{above table lists things other than pipeline stages}
...
knobs, there is one knob for each core type (small, medium and large).\ignore{
  and it basically sets the length of {\textit q\_fe }} \TODO {Different fetch
    policies can be implemented using the factory mechanism explained in
      Section~\ref{}. } The default fetch policy is round-robin and the policy
      to be used is specified using the \TODO{....} knobs. Different branch
...
\ignore{ Different fetch polices are implemented using virtual function.
  {\textit fetch\_factory} function sets different {\textit frontend\_c}.
  Currently, a thread is selected based on the round-robin policy. If the next
    thread is blocked to fetch, the next candidate is selected based on the
    round-robin policy.  Different branch predictor policies are also
    implemented and it is also set by {\textit bp\_factory}. The type of branch
    predictor or the fetch polices are set by knobs.}
...
\TODO{Decode is done in frontend.cc!}
\TODO{renaming!!}

\TODO{according to code, frontend\_q latency = fetch\_latency + alloc\_latency}
\TODO{according to code, alloc\_q latency = alloc\_to\_exec\_latency}
...
\TODO{Branch target is resolved in this stage as well.}}
...
\item \TODO{The number of function units can be specified via these knobs for
  the core type being used - \textit{int\_sched\_rate, mem\_sched\_rate,
    fp\_sched\_rate} - code does not use these in that sense}.
...
is freed or returned to memory pools. This stage also checks of completion of
warps, blocks, kernels and the entire application itself.



\TODO{At present, there are no rename and writeback stages in MacSim since
